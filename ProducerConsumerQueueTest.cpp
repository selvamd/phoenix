...
struct PerfTest {
  ...
  void operator()() {
    ...
    std::cout << "msg count " << traits_.limit() << std::endl;
...
void perfTestType(const char* type) {
  ...
  std::cout << "Type: " << type;
  doTest<PerfTest<ProducerConsumerQueue<T>,size,Pop> >(
...
void correctnessTestType(const std::string& type) {
  std::cout << "Type: " << type;
  doTest<CorrectnessTest<ProducerConsumerQueue<T>,0xfffe,Pop> >(
    "ProducerConsumerQueue");
}
...
struct DtorChecker {
  ...
  DtorChecker(const DtorChecker&) { ++numInstances; }
  ~DtorChecker() { --numInstances; }
};
...
TEST_F(PCQ, Destructor) {
  // Test that orphaned elements in a ProducerConsumerQueue are
  // destroyed.
  {
    ProducerConsumerQueue<DtorChecker> queue(1024);
    for (int i = 0; i < 10; ++i) {
      EXPECT_TRUE(queue.write(DtorChecker()));
    }

    EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)10);
    ...
    EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)8);
  }

  EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)0);

  // Test the same thing in the case that the queue write pointer has
  // wrapped, but the read one hasn't.
  {
    ProducerConsumerQueue<DtorChecker> queue(4);
    for (int i = 0; i < 3; ++i) {
      EXPECT_TRUE(queue.write(DtorChecker()));
    }
    EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)3);
    {
      DtorChecker ignore;
      EXPECT_TRUE(queue.read(ignore));
    }
    EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)2);
    EXPECT_TRUE(queue.write(DtorChecker()));
    EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)3);
  }
  EXPECT_EQ(DtorChecker::numInstances, (const unsigned long)0);
...
TEST_F(PCQ, EmptyFull) {
  ProducerConsumerQueue<int> queue(3);
  ...
  EXPECT_EQ(queue.sizeGuess(), (const unsigned long)2);
}
